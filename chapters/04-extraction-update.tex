...
%\begin{itemize}
%    \item kafka tem algumas appllicação ja devenvolvida que tiram dados de uma source e metem mensagem no kafka
...
%    \item a tabela implica ter esses dados guardados sempre. em differentes runs de achilles (em que os dados têm de ser apagados) eventos falsos de deleção seriam enviados
%\end{itemize}
...
%\begin{enumerate}
%    \item docker to facilitae deploymente of agent (will require a little intro to docker)
...
\section{Publishing}
%\begin{itemize}
%    \item data is in kafka. what do we have to do to reach the applications
%    \item idealmente usamos o kafka e afins para tudo, não tendo de desenvolver nada
...
%\begin{itemize}
%    \item centralized entity working on top of kafka, dealing with the unbounded format of data in Kafka
%    \item performs operations on top of kafka
...
As gathering all the uploaded data in memory is not a scalable option, and implementing a system with memory usage management is a complex task, the system must first filter out unnecessary data and only allow the user to build and send the request to the applications.
...
One is in charge of counting and sending the data that comply with the filtering condition and the other is counting the number of messages that do not fit the filter condition.
...
Note that the "All Data" topic is unique to the Filter Worker application and the "Required Data 1" is unique to each filter goroutine, this means that on a Filter Worker application there can be several Filters running at the same time.
...
\begin{itemize}
    \item why do we need this. why do not the workers read directly from the database.
    ...
    \item why not use ksql? cant set offset to read from. either always read from start or always read from end
    \item why java? (only it contains a kstream api) and we use it to redirect messages from one topic to another
    \item 3 threads: one reads uploads, another redirects and creates streams of data, another closes streams once all data was transfered
\end{itemize}
...
\begin{itemize}
    \item Listens to the kafka topics that are used along the system and transforms into persistent statistics data (sql)
    \item removes this responsability from the other microservices
    \item belongs to a separete consumer group
\end{itemize}
