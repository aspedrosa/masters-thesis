...
Figure \ref{fig:overall-arch-v1} presents a high-level architecture of the desired system.
Agents are software that runs on the data owner's deployment, with the role of extracting metadata from the database and sending this metadata to the applications.
...
At the other end of the scale, the burden of extracting data is removed and the system is dependent on the data owner providing the metadata and only then the system processes the metadata.
...
A more executable solution is to have an intermediate component that receives the metadata from the agents and then distributes that data across the applications, following a centralized network architecture as it is presented in figure \ref{fig:overall-arch-v3}.
...
First, when we talk of applications we are considering only web application that expect to receive metadata through a \gls{rest} \gls{api}.
...
\begin{enumerate}
    ...
    \item The system should allow managing the destination applications:
        \begin{list}{}{}
            \item The applications to where the data of the databases are sent, should not be hard-coded, for that, the network admin should be able to perform \gls{crud} operations on such.
                The term application should be interpreted as web applications that has \gls{rest} \gls{api} endpoints available to receive data.
        \end{list}
    ...
    \item The format of the request sent with the metadata to each application must be customizable:
        \begin{list}{}{}
            \item This is important since different applications expect to receive metadata in distinct formats since each application has a different \gls{api}s.
        \end{list}
...
\begin{enumerate}
    \item The data owner can stop the agent at any time:
        \begin{list}{}{}
          ...
          If at any time, they do not want to share their data, they can stop the agent.
    ...
    \item The agent should be easily deployable:
        \begin{list}{}{}
...
\begin{enumerate}
    \item Set up the agent on the local deployment system:
        \begin{list}{}{}
            \item Since it is not expected that data owners provide access to their deployment environment, the responsibility to set up the agent is imposed on them.
        \end{list}
...
Regarding the extraction tool, from the ones presented in chapter \ref{chapter:background}, the most indicated for our use case is \gls{achilles}, developed by \gls{ohdsi}, or a variation of such.
...
To make use of the package, the user must provide the information to connect to the database, such as user and password, an output directory to export the results, and the name of three schemas, which are, in most \gls{rdbms}s, a way to group tables within a database.
...
\begin{itemize}
    ...
    \item Vocabularies: as each clinical database certainly will contain their distinct internal representation of clinical concepts, such as a database might encode sex with just a character (f, m, o), others might encode with the whole word (female, male, other).
        The \gls{omop} \gls{cdm} makes use of standardized vocabularies that allow creating a homogeneous representation for different databases.
...
This can be interpreted as following the publish-subscribe pattern.
This solution allows answering the ``Microservice architecture'' non-function requirement as it helps decoupling services since they do not require to be aware of each other.
...
A well know and widely used implementation of such systems is Kafka.
Originally, the intent was to use a publish-subscribe system just for the communication between the agents and the central component.
However, Kafka has some functionalities that appeal to also use it to manage metadata, working as a central component.

Kafka was built at LinkedIn after they tested other messaging alternatives and concluded that they were designed to target low-latency use cases instead of their needed high-volume scale-out deployment.
...
In contrary to the regular publish-subscribe system, the concept of a consumer in Kafka is generalized to be a group of co-operating processes running as a cluster.
Because of that, a message in a partition of a topic is delivered to only one consumer of a group.
With this, partitions allow to control the level of parallelism of consumers, as different partitions of a topic are assigned to different consumers of a consumer group.
...
\begin{itemize}
    \item It partitions up data so that the production, brokering, and consumption of such can be handled by a cluster of entities that can be easily scaled as load increases, making it well suited for our use case;
    \item Messages are not removed from the log, instead, they can be replayed by consumers, allowing to easily build a fault-tolerant system;
...
Due to the popularity of Kafka, several libraries and frameworks were developed to extend its functionality.
...
The different connectors available range from known database systems (MongoDB, InfluxDB, etc.) to internet protocols (\gls{http}, \gls{ftp}, etc.).
    ...
    \item \gls{jdbc} Source connector\footnote{\url{https://www.confluent.io/hub/confluentinc/kafka-connect-jdbc}}:
        ...
        \begin{list}{}{}
            ...
            \item The other mode available is bulk, which sends all the rows of a table in one go.
                However, this process is not performed based on updates on data, instead, it is executed periodically on a configured interval.
                Furthermore, no feedback is provided after all the data is present in Kafka.
        \end{list}

    \item FileSystem Source connector\footnote{\url{https://github.com/mmolimar/kafka-connect-fs}}:
...
\begin{itemize}
    ...
    \item The other component is the Health Check Handler.
        ...
        The most popular and used ones are Python and Java, however, other programming languages appeared and are starting to gain terrain on web app development.
...
This problem will always exist due to the nature of Kafka, where topics are treated as streams of data, which do not have determined start and end, and there is always data flowing.
...
% scalable: several filter worker applications
...
A Filter operation will remove entire rows, however, only a subset of columns might be required.
For that, the system should also allow choosing which columns of the metadata are required.

That's what the Filter Worker component is supposed to do.
...
This can be achieved by creating two streams with opposite filtering conditions (figure \ref{fig:filter}).
\begin{figure}[H]
    \center
...
\begin{itemize}
    \item Kafka Streams: A Java library to process and analyze data that is already present on Kafka.
        ...
        Developing such an environment in Go is much easier as concurrency mechanisms are built in the language itself, and also if the number of filters gets high, threads might start to be slow and use a lot of memory, as each has its own address space.
...
However, two distinct applications might benefit from the same filtering condition.
For that, the relationship between filters on data for an application is a one-to-many relationship, so the data resulting from one filter might be used to build and send data to several applications.
...
One is in charge of counting the data that comply with the filtering condition and the other counting the number of messages that do not fit the filter condition.
...
Such a thing is not achievable with KsqlDB since it only allows to read either from the latest or earliest record.
As an alternative, Kafka Streams allows achieving this.
This will require that this redistribution of work has to be written in Java, but as an advantage removes this responsibility of reading data from databases' topics from the Filter Workers components, allowing for it to be a more concrete and small component.
...
\begin{figure}[H]
    \center
    \includegraphics[width=.9\textwidth]{orchestrator-role}
...
\begin{figure}[H]
    \center
    \includegraphics[width=.55\textwidth]{filter-worker-uploads}
...
An interface to build \gls{http} requests would require several components to allow customizations of the several items of an \gls{http} request: authorization, headers, method, etc.
...
The Admin area will allow performing the several use cases defined on the requirement analysis stage: managing communities, databases, filters and target applications, and checking statistics of the system.
...
One of the available technologies to use is Django, which must be used alongside the framework called Django REST framework\footnote{\url{https://www.django-rest-framework.org/}}, which allows exposing Django's models through an \gls{api}.
...
There is no need to implement calls to the \gls{api} exposed by the backend app, as the React-admin framework will take care of this automatically.
...
Appendix \ref{appendix:b} presents several screenshots of the Admin Portal.
...
This chapter explained the entire development process around a system capable of automating the procedure of updating metadata stored on web applications about clinical databases.
A solution was provided to the data owners to automatize the process of extracting metadata from their databases and then the system will make sure that data will reach the desired web applications.
